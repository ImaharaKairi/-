...
int main() {
    ...
    // 依次进行性能评测
    benchmark_func("V0: 朴素实现", sgemm_v0, N, A.data(), B.data(), C.data());
    benchmark_func("V1: 循环重排", sgemm_v1, N, A.data(), B.data(), C.data());
    benchmark_func("V2: 循环展开", sgemm_v2, N, A.data(), B.data(), C.data());
    benchmark_func("V3: AVX2向量", sgemm_v3_avx2, N, A.data(), B.data(), C.data());
    benchmark_func("V4: 缓存分块", sgemm_v4_tiling, N, A.data(), B.data(), C.data());
    benchmark_func("V5: 终极多核", sgemm_v5_openmp, N, A.data(), B.data(), C.data());
